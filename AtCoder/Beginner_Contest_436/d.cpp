...
ll h, w;
char grid[maxw][maxw];
bool vis[maxw][maxw], vis_p[26];

struct state {
    ll r, c, s;
};

inline ll bfs(ll r, ll c, ll tr, ll tc, vector<ll> &dist) {
    ...
    while(!q.empty()) {
        ...
        char ch = grid[s.r][s.c];
        if(ch >= 'a' && ch <= 'z') {
            ll id = ch - 'a';
            dist[id] = min(dist[id], s.s);
...
void main_algo() {
    ...
    rep(i, 1, h) {
        string s;
        cin >> s;
        memcpy(grid[i] + 1, s.c_str(), w);
        // cin >> (grid[i] + 1);
    }

    ll ans = inf;
    vector<ll> d1(26, inf), d2(26, inf);
    
    ans = min(bfs(1, 1, h, w, d1), ans);
    // ans = min(bfs(h, w, 1, 1, d2), ans);

    // rep(i, 1, 26) {
    //     cout << d1[i - 1] << " " << d2[i - 1] << endl;
    // }

    // rep(i, 1, 26) {
    //     if(d1[i - 1] == inf || d2[i - 1] == inf) continue;
    //     ans = min(ans, d1[i - 1] + d2[i - 1] - 1);
    // }
