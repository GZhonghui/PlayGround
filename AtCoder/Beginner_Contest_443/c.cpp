...
void main_algo() {
    ...
    ll idx = 1, next_open = -1, ans = 0, now = 0;
    auto push_p = [&](ll p) {
        if(next_open == -1) next_open = p;        
    };
    auto next_p = [&](pair<ll, bool> &res) -> bool {
        if(idx > n + 1) return false;

        if(next_open == -1 || a[idx] < next_open) res = {a[idx++], false};
        else {
            res = {next_open, true};
            next_open = -1;
        }
    ...
    pair<ll, bool> res;
    while(next_p(res)) {
        // cout << res.first << ", " << (res.second ? "T" : "F") << endl;
        if(res.second) {

        } else {
            if(next_open == -1) ans += res.first - now;
            push_p(res.first + 100);
        }
        now = res.first;
    }

    // ll ans = 0, now = 0, next_open = -1;
    // rep(i, 1, n + 1) {
    //     if(next_open == -1) {
    //         ans += a[i] - now;
    //         next_open = a[i] + 100;
    //         now = a[i];
    //     } else if (a[i] < next_open) {
    //         now = a[i];
    //     } else {
    //         ans += a[i] - next_open;
    //         now = a[i];
