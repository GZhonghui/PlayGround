...
vector< vector<ll> > memo(maxw, vector<ll>(maxw, -1));

ll dp(ll w_limit, ll w_sum) {
    if(memo[w_limit][w_sum] != -1) return memo[w_limit][w_sum];
    ll &res = memo[w_limit][w_sum];

    if(w_limit == 0) return res = 0;

    res = 0;
    priority_queue<ll> pq;
    for(auto &i : items[w_limit]) {
        pq.push(i.v - 1);
    }

    ll k_limit = w_sum / w_limit, _sum = 0;
    rep(k, 0, k_limit) {
        res = max(res, _sum + dp(w_limit - 1, w_sum - k * w_limit));
        if(!pq.empty()) {
            ll _v = pq.top();
            pq.pop();
            _sum += _v;
...
int main()
{
    ...
    rep(i, 1, n) {
        cin >> _item.w >> _item.v;
        max_w = max(max_w, _item.w);
        items[_item.w].pb(_item);
    }

    ll res = 0;
    rep(i, 1, w) {
        res = max(res, dp(max_w, i));
    }
