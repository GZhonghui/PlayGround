...
inline bool check() {
    ...
    ll moves = cnt[0] * cnt[1] - (n - 1);

    first = moves & 1;

    cout << (first ? "First" : "Second") << endl << flush;
    return true;
    // return moves & 1;
}

void main_algo() {
    unordered_set<ll> connected[maxn];
    ...
    rep(i, 1, n - 1) {
        ...
        connected[min(u, v)].insert(max(u, v));
    }
    ...
    while(true) {
        ...
        auto me = [&]() {
            rep(i, 1, n) {
                rep(j, i + 1, n) {
                    if(!connected[i].count(j) && depth[i] % 2 != depth[j] % 2) {
                        cout << i << " " << j << endl << flush;
        ...
        auto op = [&]() -> bool {
            ...
            // accept the move
            connected[min(i, j)].insert(max(i, j));
            return false;
        };
