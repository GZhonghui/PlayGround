...
ll dfs(ll u, ll fa) {
    ...
    // vector<ll> remains;
    // ll l = 0, r = todos.size() - 1;
    // while(l <= r) {
    //     ll left = todos[l];
    //     while(r > l && todos[r] + left > k) {
    //         remains.pb(todos[r]);
    //         r--;
    //     }
    //     if(l >= r || left + todos[r] != k) {
    //         remains.pb(left);
    //     } else {
    //         r--;
    //     }
    ...
    if(todos.size() == 0) return 1 % k;
    if(todos.size() == 1) return (todos[0] + 1) % k;
    if(todos.size() == 2) {
        if(todos[0] + todos[1] + 1 == k) return 0;
        else return -1;
    }
    return -1;
}
