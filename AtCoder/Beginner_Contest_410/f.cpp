...
static ll cnt[maxn<<1], tag[maxn<<1];
static ll timer = 0;
...
inline void solve() {
    ...
    ll hh = min(h, w), ww = max(h, w);

    vector< vector<ll> > data = vector< vector<ll> >(hh + 1, vector<ll>(ww + 1, 0));
    vector< vector<ll> > hsum = vector< vector<ll> >(hh + 1, vector<ll>(ww + 1, 0));
    if(h == hh && w == ww) {
        rep(i, 1, hh) {
            rep(j, 1, ww) {
                data[i][j] = in[i - 1][j - 1] == '#' ? -1 : 1;
            }
        }
    } else {
        rep(i, 1, hh) {
            rep(j, 1, ww) {
                data[i][j] = in[j - 1][i - 1] == '#' ? -1 : 1;
            }
        }
    }
    // rep(i, 1, hh) {
    ...
    rep(j, 1, ww) {
        rep(i, 1, hh) {
            hsum[i][j] = data[i][j] + hsum[i - 1][j];
        }
    }

    vector<ll> line(ww + 1, 0);
    rep(h1, 1, hh) rep(h2, h1, hh) {
        rep(j, 1, ww) {
            line[j] = hsum[h2][j] - hsum[h1 - 1][j];
        }
        // cout << h1 << " <-> " << h2 << endl;
        // rep(j, 1, ww) {
        //     cout << line[j] << " ";
