...
ll dp(ll cnt, ll remain_h) {
    ...
    if(v[cnt][remain_h]) {
        return memo[cnt][remain_h];
    }
    v[cnt][remain_h] = true;
    ll &res = memo[cnt][remain_h] = -inf;

    auto [a, b] = ms[cnt];

    ll need_h = remain_h + a;
    if(need_h <= h) {
        res = max(res, dp(cnt - 1, need_h));
    }

    res = max(res, dp(cnt - 1, remain_h) - b);
    return res;
}
