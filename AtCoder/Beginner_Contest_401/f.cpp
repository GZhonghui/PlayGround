...
void main_algo() {
    ...
    vector<S_both> seg_data(n2);
    for (ll i = 0; i < n2; i++) {
        seg_data[i] = {B[i], B[i], 1};
    }
    lazy_segtree<S_both, op_both, e_both, F_both, mapping_both, composition_both, id_both> seg(seg_data);

    ll sum_all = seg.prod(0, n2).sum;

    for (ll i = 1; i <= n1; i++) {
        ll T = original_max - 1 - dist1[i];

        ll pos = (ll)( upper_bound(B.begin(), B.end(), T) - B.begin() ) - 1;

        if (pos >= 0) {
            ans += (pos + 1) * original_max;

            ll cnt = n2 - (pos + 1);
            if (cnt > 0) {
                ll sum_sub = seg.prod(pos + 1, n2).sum;
                ans += (dist1[i] + 1) * cnt + sum_sub;
            }
        }
        else {
            ans += n2 * (dist1[i] + 1) + sum_all;
        }
    }
