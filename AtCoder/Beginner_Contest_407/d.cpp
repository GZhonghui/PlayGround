    ...
    rep(i, 1, h) {
        rep(j, 1, w) {
            ...
            if(b & ((ll)1 << (_idx - 1))) {
                ...
                res = max(res, a[i][j] ^ dp(b | ((ll)1 << (_idx - 1))));
                // res = a[i][j] ^ dp(b | ((ll)1 << (_idx - 1)));
...
void dfs(ll idx, ll used, ll cur) {
    while(idx < h * w && (used >> idx & 1)) ++idx;
    if(idx == h * w) {
        best = max(best, cur);
        return;
    }

    dfs(idx + 1, used | ((ll)1 << idx), cur ^ a[idx / w + 1][idx % w + 1]);

    ll r = idx % w, d = idx / w;
    if(r + 1 < w && !(used >> (idx + 1) & 1))
        dfs(idx + 1, used | ((ll)1 << idx) | ((ll)1 << (idx + 1)), cur);
    if(d + 1 < h && !(used >> (idx + w) & 1))
        dfs(idx + 1, used | ((ll)1 << idx) | ((ll)1 << (idx + w)), cur);
}
