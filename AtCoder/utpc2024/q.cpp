...
struct node {
    ...
    void calc() {
        // a = ((lll)ai2 - 2 * (lll)ai1 + (lll)ai) * (f)0.5;
        // b = (lll)ai1 - (lll)ai - (1 + 2 * (lll)i) * a;
        // c = (lll)ai - a * (lll)i * (lll)i - b * (lll)i;
        a2 = (lll)ai2 - 2 * (lll)ai1 + (lll)ai;
        b2 = ((lll)ai1 - (lll)ai) * 2 - (1 + 2 * (lll)i) * a2;
        c2 = (lll)ai * 2 - a2 * (lll)i * (lll)i - b2 * (lll)i;
    }

    bool operator==(const node &r) const {
        // return abs(a - r.a) < eps && abs(b - r.b) < eps && abs(c - r.c) < eps;
        return a2 == r.a2 && b2 == r.b2 && c2 == r.c2;
    }
};
...
int main()
{
    ...
    while(t--) {
        ...
        ll ans = 1;
        node now = {1, a[1], a[2], a[3]};
        now.calc();
        // cout << 1 << ", ";

        rep(i, 4, n) {
            node next = {i - 2, a[i - 2], a[i - 1], a[i]};
            next.calc();
            if(!(next == now)) {
                ans++;
