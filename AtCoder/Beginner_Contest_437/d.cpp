...
void main_algo() {
    ...
    ll sum_b_l = 0, cnt_b_l = 0, sum_b_r = 0, cnt_b_r = 0;
    rep(i, 1, n) {
        while(cnt_b_l < m && b[cnt_b_l + 1] <= a[i]) {
            cnt_b_l += 1;
            sum_b_l = (sum_b_l + b[cnt_b_l]) % mod;
        }
        ans = (ans + (a[i] * cnt_b_l % mod - sum_b_l + mod) % mod) % mod;
    }
    rre(i, 1, n) {
        while(cnt_b_r < m && b[m - cnt_b_r] > a[i]) {
            sum_b_r = (sum_b_r + b[m - cnt_b_r]) % mod;
            cnt_b_r += 1;
        }
        ans = (ans + (sum_b_r - a[i] * cnt_b_r % mod + mod) % mod) % mod;
    }
