...
#endif
    cin >> n;
    number_theory::solver.init();
    number_theory::solver.calc(sqrt(n) + 1);

    ll ans = 0;
    bool running = true;
    for(ll i = 0; i < number_theory::solver.prime.size(); ++ i) {
        ll x = number_theory::solver.prime[i];
        if(running) {
            bigint::bigint _x(x);
            bigint::bigint x8 = _x.fast_pow(8);
            if(x8 > n) running = false;
            else ans ++;
        }
        
        bigint::bigint _x(x);
        // if(_x.fast_pow(4) > n) break;

        for(ll j = i + 1; j < number_theory::solver.prime.size(); ++ j) {
            ll y = number_theory::solver.prime[j];

            bigint::bigint _y(y);
            if(_x * _x * _y * _y <= n) { ans ++; }
            else break;
        }
    }
