...
void main_algo() {
    ...
    vector<S_both> initial;
    initial.resize(n + 1, {0, 0, 1});
    rep(i, 1, n) {
        initial[i].sum = a[i];
        initial[i].max = a[i];
    }
    ...
    rep(i, 1, m) {
        ...
        ll sum = seg.prod(l, r + 1).sum % mod;
        ll len = r - l + 1;
        ll nxt = sum * inverse_element::inverse_element_exgcd(len, mod) % mod;
        seg.apply(l, r + 1, F_both{0, nxt, true});
    }
