...
ll h, w, k, dp[maxn][maxn], dis[maxn][maxn][4], cache[4];
...
void main_algo() {
    ...
    rep(i, 1, h) {
        rep(j, 1, w) {
            dp[i][j] = (target[i][j] ? 0 : inf);
            rep(t, 0, 3) dis[i][j][t] = inf;
        }
    }
    ...
    while(!Q.empty()) {
        ...
        rep(i, 0, 3) {
            ...
            dis[x][y][i] = min(dis[x][y][i], now.dis + 1);
            // sort(dis[x][y], dis[x][y] + 4);
            rep(t, 0, 3) cache[t] = dis[x][y][t];
            sort(cache, cache + 4);
