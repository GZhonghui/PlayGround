...
inline void solve() {
    ...
    rep(i, 1, n) {
        cin >> nums[i - 1];
        if(nums[i - 1] > 0) pos.pb(nums[i - 1]);
        else nev.pb(nums[i - 1]);
    }
    sort(pos.begin(), pos.end());
    sort(nev.begin(), nev.end(), [](ll a, ll b) {
        return a > b;
    });
    ...
    auto test = [&]() -> bool {
        rep(i, 1, fin.size() - 2) {
            if(fin[i] * fin[i] != fin[i - 1] * fin[i + 1]) {
                return false;
            }
        }
        return true;
    ...
    if(pos.size() == n) {
        for(ll i : pos) fin.pb(i);
    } else if(nev.size() == n) {
        for(ll i : nev) fin.pb(i);
    } else if(pos.size() == nev.size()) {
        bool res = false;
        vector<ll> *l, *s;
        ...
        return;
    } else if(abs((int)(pos.size() - nev.size())) <= 1) {
        vector<ll> *l, *s;
