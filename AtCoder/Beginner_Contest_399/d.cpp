...
void main_algo() {
    ...
    while(t--) {
        ...
        rep(i, 1, (n << 1)) {
            ...
            if(i <= (n << 1) - 1 && a[i] == a[i + 1]) {
                continue;
            }
            if(i - 1 >= 1 && !linkded[a[i]].count(i - 1)) {
                if(!s[a[i]].count(a[i - 1])) {
                    s[a[i]][a[i - 1]] = 0;
                }
                s[a[i]][a[i - 1]]++;
                linkded[a[i]].insert(i - 1);
            }
            if(i + 1 <= (n << 1)) {
                if(!s[a[i]].count(a[i + 1])) {
                    s[a[i]][a[i + 1]] = 0;
        ...
        rep(i, 1, n) {
            for(auto x : s[i]) {
                if(x.second == 2 && s[x.first].count(i) && s[x.first][i] == 2) {
                    // cout << "ok: " << i << " " << x.first << endl;
                    ans++;
                }
            }
        }
