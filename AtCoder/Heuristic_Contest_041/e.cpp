...
void find_nodes_in_this_subtree(ll x, ll f, ll depth) {
    ...
    for(ll to : subg[x]) {
        if(to == f) continue;
        find_nodes_in_this_subtree(to, x, depth + 1);
    }
}

void calc_ans_if_x_is_root(ll x, ll f, ll depth, ll &this_ans, bool &depth_is_ok) {
    this_ans += (depth + 1) * a[x];

    if(depth > h) depth_is_ok = false;

    for(ll son : subg[x]) {
        if(son == f) continue;
        calc_ans_if_x_is_root(son, x, depth + 1, this_ans, depth_is_ok);
    }
}
...
void rebuild_tree(ll root) {
    ...
    find_nodes_in_this_subtree(root, -1, 0);

    for(ll try_root : nodes_in_this_subtree) {
        ll this_ans = 0;
        bool depth_is_ok = true;
        calc_ans_if_x_is_root(try_root, -1, 0, this_ans, depth_is_ok);
