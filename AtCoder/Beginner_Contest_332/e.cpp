...
vector<ll> memo;
void dfs(ll idx, f now, f least) {
    if(idx > n) {
        ll last = d - memo.size();
        ans = min(ans, now + last * avg * avg);
        return;
    }

    if(least > ans + 1e-6) return;

    rep(i,1,memo.size()) {
        f _t1 = memo[i - 1] - avg;
        f _t2 = memo[i - 1] + w[idx] - avg;
        f _flow1 = max(_t1, (f)0);
        f _flow2 = max(_t2, (f)0);
        memo[i - 1] += w[idx];
        dfs(idx + 1, now - _t1 * _t1 + _t2 * _t2, least - _flow1 * _flow1 + _flow2 * _flow2);
        memo[i - 1] -= w[idx];
    }
    ...
    if(memo.size() < d) {
        ...
        f _t = w[idx] - avg;
        f _flow = max(_t, (f)0);
        dfs(idx + 1, now + _t * _t, least + _flow * _flow);
        memo.pop_back();
    }
}
