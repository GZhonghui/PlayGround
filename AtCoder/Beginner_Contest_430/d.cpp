...
S_both e_both() {
    return S_both{0, (ll)-1e18, (ll)1e18, 0};
}
...
ll x[maxn], xx[maxn], l[maxn], r[maxn];
unordered_map<ll, ll> x_to_rank, rank_to_x;

unordered_map<ll, ll> dist;
ll ans = 0;

inline void update(ll rank) {
    ll n_dist = min(
        l[rank] == -1 ? inf : abs(rank_to_x[rank] - rank_to_x[l[rank]]),
        r[rank] == -1 ? inf : abs(rank_to_x[rank] - rank_to_x[r[rank]])
    );
...
void main_algo() {
    ...
    vector<S_both> initial_l(n + 8, {0, 0, 0, 1});
    lazy_segtree<S_both, op_both, e_both, F_both, mapping_both, composition_both, id_both> seg_l(initial_l);
    vector<S_both> initial_r(n + 8, {inf, inf, inf, 1});
    lazy_segtree<S_both, op_both, e_both, F_both, mapping_both, composition_both, id_both> seg_r(initial_r);

    rep(i, 1, n) {
        ll rank = x_to_rank[x[i]];

        ll left_node = seg_l.prod(0, rank).max;
        ll right_node = seg_r.prod(rank + 1, n + 1).min;

        seg_l.set(rank, {rank, rank, rank, 1});
        seg_r.set(rank, {rank, rank, rank, 1});

        l[rank] = left_node;
        r[rank] = right_node <= n ? right_node : -1;
        update(rank);

        // cout << "rank: " << rank << ", l: " << l[rank] << ", r: " << r[rank] << endl;
        // cout << "ans = " << ans << endl;
