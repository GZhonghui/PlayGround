...
void main_algo() {
    ...
    vector<S_both> initial_a(n + 1, {0, 0, 1});
    vector<S_both> initial_b(n + 1, {0, 0, 1});
    lazy_segtree<S_both, op_both, e_both, F_both, mapping_both, composition_both, id_both> seg_a(initial_a);
    lazy_segtree<S_both, op_both, e_both, F_both, mapping_both, composition_both, id_both> seg_b(initial_b);

    rep(i, 1, n) {
        if(s[i - 1] == 'a') {
            seg_a.apply(i, i + 1, F_both{1, 0, false});
        } else {
            seg_b.apply(i, i + 1, F_both{1, 0, false});
        }
    }

    ll ans = 0;
    rep(l, 1, n) {
        ll from = seg_a.max_right(l, [](S_both x) {
            return x.sum < a;
        });
        ll to = seg_b.max_right(l, [](S_both x) {
            return x.sum < b;
        }) - 1;

        ans += max((ll)0, to - from + 1);
    }
