...
void main_algo() {
    ...
    // vector<S_both> initial(2 * n + 8, {0, 0, 1});
    // lazy_segtree<S_both, op_both, e_both, F_both, mapping_both, composition_both, id_both> left(initial);
    // lazy_segtree<S_both, op_both, e_both, F_both, mapping_both, composition_both, id_both> right(initial);

    // rep(i, 1, m) {
    //     left.apply(as[i], F_both{1, 0, false});
    //     right.apply(bs[i], F_both{1, 0, false});
    // }
    ...
    rep(i, 1, q) {
        ...
        // {
        //     ll t1 = left.prod(0, c).sum;
        //     ll t2 = right.prod(0, d).sum;
        //     ans += max((ll)0, t1 + t2 - m);
        // }

        // {
        //     ll t1 = m - left.prod(0, c).sum;
        //     ll t2 = m - right.prod(0, d).sum;
        //     ans += max((ll)0, t1 + t2 - m);
        // }
        // cout << ans << endl;
    }

    {
        ...
        ll line_idx = 0;
        for(query &t : qs) {
            while(line_idx < lines.size() && lines[line_idx].to < t.c) {
                seg.apply(lines[line_idx].from, F_both{1, 0, false});
                line_idx += 1;
            }
            ans[t.id] += seg.prod(t.c, t.d).sum;
        }
    }

    {
        ...
        ll line_idx = 0;
        for(query &t : qs) {
            while(line_idx < lines.size() && lines[line_idx].to > t.d) {
                seg.apply(lines[line_idx].from, F_both{1, 0, false});
                line_idx += 1;
            }
            ans[t.id] += seg.prod(t.c, t.d).sum;
