...
void main_algo() {
    ...
    while(!Q.empty()) {
        ...
        if(now.dis >= d[now.x][now.y]) {
            continue;
        }

        d[now.x][now.y] = now.dis;
        ans[now.x][now.y] = now.from;

        rep(k, 0, 3) {
            ll xx = now.x + dx[k], yy = now.y + dy[k];
            if(1 <= xx && xx <= h && 1 <= yy && yy <= w && m[xx][yy] == '.' && now.dis + 1 < d[xx][yy]) {
                char from = 0;
                if(k == 0) from = 'v';
                if(k == 1) from = '<';
                if(k == 2) from = '^';
                if(k == 3) from = '>';
                Q.push((node){xx, yy, now.dis + 1, from});
            }
        }
    }
