...
void main_algo() {
    ...
    ll ps_size = ps.size();
    unordered_map<ll, ll> ps_id;
    vector<S_both> initial(ps_size + 8, {0, 0, 1});
    rep(i, 1, ps_size) {
        ps_id[ps[i - 1]] = i;

        if(i > 1) {
            initial[i].sum = initial[i].max = ps[i - 1] - ps[i - 2];
        }
    }
    ...
    rep(i, 1, q) {
        ll l = queries[i - 1].first, r = queries[i - 1].second;
        seg.apply(ps_id[l - 1] + 1, ps_id[r] + 1, F_both{0, 0, true});
        cout << seg.prod(0, ps_size + 1).sum << endl;
    }
}