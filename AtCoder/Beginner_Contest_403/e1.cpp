...
ll q, t, cnt[maxn], end_point[maxn];
bool block[maxn];
string s;
...
void main_algo() {
    ...
    while(q--) {
        ...
        if(t == 1) {
            ...
            if(can_insert) {
                solver.insert(s, [](ll id) {
                    // cnt[solver.f(id)] -= cnt[id];
                }, [](ll id) {
                    cnt[id] = 0;
                    block[id] = true;
                }, [](ll id) {
                    cnt[solver.f(id)] = end_point[solver.f(id)];
                    solver.s(solver.f(id), [](ll son) {
                        cnt[solver.f(son)] += cnt[son];
                    });
                });
            }
        } else if(t == 2) {
