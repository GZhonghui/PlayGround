...
ll memo[maxn][2];
ll dp(ll n, uint8_t is_add) {
    if(memo[n][is_add] != -1) {
        return memo[n][is_add];
    }

    ll &res = memo[n][is_add] = inf;
    ...
    if(is_add) {
        rep(left, 1, n - 1) {
            ll right = n - left;
            res = min(res, dp(left, 0) + dp(right, 0) + 1); 
            res = min(res, dp(left, 1) + dp(right, 0) + 1);
            res = min(res, dp(left, 0) + dp(right, 1) + 1);
            res = min(res, dp(left, 1) + dp(right, 1) + 1);
        }
    } else {
        rep(left, 2, n >> 1) {
            if(n % left) continue;
            ll right = n / left;
            res = min(res, dp(left, 0) + dp(right, 0) + 1);
            res = min(res, dp(left, 1) + dp(right, 0) + 3);
            res = min(res, dp(left, 0) + dp(right, 1) + 3);
            res = min(res, dp(left, 1) + dp(right, 1) + 5);
        }
    }
    return res;
}

void print(ll n, uint8_t is_add) {
    ll ans = dp(n, is_add);

    ll cnt = count(n);
    if(cnt && cnt == ans) {
        rep(i, 1, cnt) printf("1");
        return;
    }
    ...
    if(is_add) {
        rep(left, 1, n - 1) {
            ll right = n - left;
            if(ans == dp(left, 0) + dp(right, 0) + 1) {
                print(left, 0);
                printf("+");
                print(right, 0);
                break;
            }
            if(ans == dp(left, 1) + dp(right, 0) + 1) {
                print(left, 1);
                printf("+");
                print(right, 0);
                break;
            }
            if(ans == dp(left, 0) + dp(right, 1) + 1) {
                print(left, 0);
                printf("+");
                print(right, 1);
                break;
            }
            if(ans == dp(left, 1) + dp(right, 1) + 1) {
                print(left, 1);
                printf("+");
                print(right, 1);
                break;
