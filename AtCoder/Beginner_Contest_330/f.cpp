...
bool check(ll width) {
    auto check = [&](vector<ll> &sorted_data) -> ll {
        queue<ll> q, next;
        for(ll i : sorted_data) {
            next.push(i);
        }

        vector<ll> stops(n << 1);
        rep(i, 1, n) {
            stops[i - 1] = sorted_data[i - 1];
            stops[i + n - 1] = sorted_data[i - 1] + width;
        }
        sort(stops.begin(), stops.end());

        ll cost = 0, min_cost = inf, r = stops[0], outed = 0;
        rep(i, 1, n) {
            cost += max((ll)0, sorted_data[i - 1] - r);
        }

        min_cost = min(min_cost, cost);

        auto in = [&]() {
            while(!next.empty() && next.front() <= r) {
                q.push(next.front());
                next.pop();
            }
        };
        ...
        auto out = [&]() -> ll {
            ll delta = 0;
            while(!q.empty() && q.front() < r - width) {
                delta += r - width - q.front();
                outed++;
                q.pop();
            }
            return delta;
...
void main_algo() {
    ...
    ll l = 0, r = max(xs[n - 1] - xs[0], ys[n - 1] - ys[0]);
    while(r - l > 3) {
        ll mid = (l + r) >> 1;
        if(check(mid)) {
            r = mid;
