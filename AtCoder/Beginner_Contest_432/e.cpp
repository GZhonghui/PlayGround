...
F_both composition_both(F_both new_op, F_both old_op) {
    if (new_op.is_set) {
        ...
        return F_both{new_op.add + old_op.add, old_op.set + new_op.add,  old_op.cnt + new_op.cnt, true};
    } else {
        return F_both{new_op.add + old_op.add, 0, old_op.cnt + new_op.cnt, false};
    }
}

F_both id_both() {
    return F_both{0, 0, 0, false};
}

inline void lazy_segtree_toolkit() {
    vector<S_both> initial(8, {0, 0, 0, 1}); // avaliable range: [0, 8)
    ...
    seg.apply(1, 5, F_both{3, 0, 0, false}); // [1, 5)
...
void main_algo() {
    ...
    vector<S_both> initial(maxn, {0, 0, 0, 1}); // avaliable range: [0, 8)
    rep(i, 0, maxn - 1) {
        initial[i].sum = sum[i];
        initial[i].max = sum[i];
        initial[i].cnt = cnt[i];
    ...
    while(q--) {
        ...
        if(t == 1) {
            auto xx = seg.get(a[x]);
            seg.set(a[x], {xx.sum - a[x], xx.max - a[x], xx.cnt - 1});
            a[x] = y;
            auto yy = seg.get(a[x]);
            seg.set(a[x], {yy.sum + y, yy.max + y, yy.cnt + 1});
        } else if(t == 2) {
            if(x <= y) {
                ll center = seg.prod(x, y + 1).sum;
                ll l = seg.prod(0, x).cnt, r = seg.prod(y + 1, maxn).cnt;
                cout << x * l + center + y * r << endl;
            } else {
                cout << x * n << endl;
            }
        }
