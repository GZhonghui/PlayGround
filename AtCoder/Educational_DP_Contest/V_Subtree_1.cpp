...
ll post_dfs(ll x, ll f, ll f_cnt = 1) {
    ans[x] = dp(x, f) * f_cnt % m;
    ...
    ll son_cnt = sons.size() - 1;

    vector<ll> l(son_cnt + 2, 1), r(son_cnt + 2, 1);

    rep(i,1,son_cnt) {
        l[i] = l[i-1] * (dp(sons[i], x) + 1) % m;
    }
    rre(i,1,son_cnt) {
        r[i] = r[i+1] * (dp(sons[i], x) + 1) % m;
    }

    rep(i,1,son_cnt) {
        ll _t = l[i-1] * r[i+1] % m * f_cnt % m;
        post_dfs(sons[i], x, (_t + 1) % m);
    }
