...
struct tower {
    ...
    bool operator<(const tower &r) const {
        ll w_self_up = min(s, r.s - w);
        ll w_other_up = min(r.s, s - r.w);
        return w_self_up > w_other_up;
    }
};
...
int main()
{
    ...
    cin >> n;
    rep(i,1,n) {
        cin >> towers[i].w >> towers[i].s >> towers[i].v;
    }
    ...
    rep(i,1,n) {
        ...
        rep(j,0,t.s) {
            _dp[j + t.w] = max(_dp[j + t.w], dp[j] + t.v);
        }
