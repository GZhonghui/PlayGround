...
int main()
{
    ...
    cin >> n >> m;
    rep(i,1,m) {
        cin >> intervals[i].l >> intervals[i].r >> intervals[i].a;
        r[intervals[i].r].pb(i);
    }

    vector<S_both> initial(n + 8, {0, 0, 1}); // avaliable range: [0, 8)
    lazy_segtree<S_both, op_both, e_both, F_both, mapping_both, composition_both, id_both> seg(initial);

    // 如何设DP状态转移方程可以参考题解 这一部分不难理解
    rep(i,1,n) {
        ll _sum = 0;
        // 这些都是新出现的区间 之前并没有计算过
        // 「出现」指的是这个区间完整进入我们的计算范围 所以使用区间右端点标记
        for(ll idx : r[i]) {
            _sum += intervals[idx].a;
        }

        // 刚出现的区间 之前肯定没有算过
        // 而这里我们选择的是1的话 那么这个区间就一定有贡献
        ll _max = seg.prod(0, i).max;
        seg.apply(i, i + 1, F_both{0, _max + _sum, true});

        // 新出现的区间 之前可能有漏算 要统一加上
        // 哪些漏算了呢？ 被这个区间包括的漏算了
        for(ll idx : r[i]) {
            const interval &t = intervals[idx];

            seg.apply(t.l, i, F_both{t.a, 0, false});
        }
    }

    cout << seg.prod(0, n + 1).max << endl;
