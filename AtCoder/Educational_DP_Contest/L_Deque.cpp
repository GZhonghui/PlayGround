...
ll dp(ll l, ll r, bool first) {
    if(v[l][r][first]) return memo[l][r][first];
    v[l][r][first] = true;
    ll &res = memo[l][r][first];

    if(l == r) {
        res = a[l] * (first ? 1 : -1);
    } else {
        if(first) {
            res = max(a[l] + dp(l+1, r, false), a[r] + dp(l, r-1, false));
        } else {
            res = min(-a[l] + dp(l+1, r, true), -a[r] + dp(l, r-1, true));
        }
    }
