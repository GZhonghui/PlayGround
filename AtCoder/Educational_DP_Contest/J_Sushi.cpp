...
f dp(ll a, ll b, ll c) {
    ...
    res = 0;
    if(a > 0) res += (dp(a-1, b, c) + 1) * a / n;
    if(b > 0) res += (dp(a+1, b-1, c) + 1) * b / n;
    if(c > 0) res += (dp(a, b+1, c-1) + 1) * c / n;
    res += 1.0 * (n - a - b - c) / n;
    res *= 1.0 * n / (a + b + c);
...
int main()
{
    ...
    cout << fixed << setprecision(20) << dp(cnt[1], cnt[2], cnt[3]) << endl;
