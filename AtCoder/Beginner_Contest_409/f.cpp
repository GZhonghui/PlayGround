...
struct edge {
    ll from, to, dis;
    bool operator<(const edge &rhx) const {
        return dis > rhx.dis;
    }
};
...
void main_algo() {
    ...
    rep(i, 1, n) {
        rep(j, i + 1, n) {
            Q.push((edge){i, j, points[i - 1].dis2(points[j - 1])});
        }
    }
    ...
    rep(_q, 1, q) {
        ...
        if(type == 1) {
            ...
            rep(i, 1, id - 1) {
                Q.push((edge){i, id, points[i - 1].dis2(points[id - 1])});
            }
            ...
            if(cnt == 1) {
                ...
                while(!Q.empty() && ds.judge(Q.top().from, Q.top().to)) {
                    Q.pop();
                }

                ll dis = Q.top().dis;
                while(!Q.empty() && Q.top().dis == dis) {
                    edge e = Q.top();
                    Q.pop();
