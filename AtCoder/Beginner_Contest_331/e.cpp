...
void main_algo() {
    ...
    vector<node> nodes_confilct[maxn];
    unordered_set<ll> conflict_set[maxn];
    ll c, d;
    rep(i, 1, l) {
        cin >> c >> d;
        nodes_confilct[c].pb((node){d, b[d]});
        conflict_set[c].insert(d);
    }

    ll ans = 0;
    rep(i, 1, n) {
        sort(nodes_confilct[i].begin(), nodes_confilct[i].end());
        rre(j, 1, m) {
            // if(!nodes_confilct[i].empty() && nodes_confilct[i].back().id == nodes[j - 1].id) {
            //     nodes_confilct[i].pop_back();
            //     continue;
            // }
            // ans = max(ans, a[i] + nodes[j - 1].v);
            // break;

            if(!conflict_set[i].count(nodes[j - 1].id)) {
                ans = max(ans, a[i] + nodes[j - 1].v);
                break;
            }
        }
