...
ll dfs(ll x, ll y) {
    ...
    return res = dfs(x - 1, y) + dfs(x, y - 1) - dfs(x - 1, y - 1) + (c[x][y] == 'B' ? 1 : 0);
}
...
inline ll sum(ll x, ll y) {
    ...
    ll x_cnt = x / n, y_cnt = y / n;
    ll x_rem = x - x_cnt * n, y_rem = y - y_cnt * n;

    ll res = s[n][n] * x_cnt * y_cnt;
    res += s[x_rem][n] * y_cnt;
    res += s[n][y_rem] * x_cnt;
    res += s[x_rem][y_rem];
    return res;
