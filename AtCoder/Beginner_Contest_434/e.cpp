...
max_flow_dinic::Dinic dinic_solver;
...
void main_algo() {
    ...
    vector<pair<ll, ll>> targets(n + 1);
    rep(i, 1, n) {
        ll x, r;
        cin >> x >> r;
        
        targets[i].first = insert(x - r);
        targets[i].second = insert(x + r);
    }

    ll m = ids.size();

    ll s = n + m + 1, t = s + 1;
    dinic_solver.init(s, t, t);
    rep(i, 1, n) {
        dinic_solver.AddEdge(s, i, 1);
        dinic_solver.AddEdge(i, n + targets[i].first, 1);
        dinic_solver.AddEdge(i, n + targets[i].second, 1);
    }
    rep(i, 1, m) {
        dinic_solver.AddEdge(n + i, t, 1);
    }

    cout << dinic_solver.maxFlow() << endl;
}