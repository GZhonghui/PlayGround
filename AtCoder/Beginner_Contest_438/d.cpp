...
void main_algo() {
    ...
    vector<ll> sum_a(n + 1, 0), sum_c(n + 1, 0);
    rep(i, 1, n) sum_a[i] = sum_a[i - 1] + a[i];
    rep(i, 1, n) sum_c[i] = sum_c[i - 1] + c[i];

    ll ans = 0, best_xy = a[1] + b[2];
    rep(xy, 2, n - 1) {
        ll cv = sum_c[n] - sum_c[xy];

        if(xy > 2) {
            best_xy = max(best_xy + b[xy], sum_a[xy - 1] + b[xy]);
        }

        ans = max(ans, best_xy + cv);
    }
