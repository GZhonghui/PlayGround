...
S_both op_both(S_both a, S_both b) {
    ll llen = a.llen, rlen = b.rlen;
    if(a.llen == a.len && a.rchar == b.lchar) {
        llen = a.llen + b.llen;
    }
    if(b.rlen == b.len && b.lchar == a.rchar) {
        rlen = b.rlen + a.rlen;
    }
    ...
    return S_both{
        ...
        llen,
        rlen,
        max(max(a.in, b.in), (a.rchar == b.lchar ? a.rlen + b.llen : 0)),
        a.len + b.len
    };
}

S_both e_both() {
    return S_both{0, 0, 0, 0, 0, 0};
}
...
void main_algo() {
    ...
    vector<S_both> initial(n + 1);

    initial[0] = S_both{0, 0, 1, 1, 1, 1};
    rep(i, 1, n) {
        initial[i] = S_both{
            s[i - 1], s[i - 1],
            1, 1, 1, 1
