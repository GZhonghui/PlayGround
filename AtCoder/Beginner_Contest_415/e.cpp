...
ll dp(ll i, ll j) {
    ...
    if(i == 1 && j == 1) {
        ll val = init + grids[1][1] - P[1];
        return res = (val >= 0 ? val : -inf);
    }

    if(i > 1) {
        ll remain = dp(i - 1, j);
        ll val = (remain + grids[i][j] - P[i + j - 1]);
        ll local = (val >= 0 ? val : -inf);
        res = max(res, local);
    }
    if(j > 1) {
        ll remain = dp(i, j - 1);
...
void main_algo() {
    cin >> h >> w;
    grids.resize(h + 1, vector<ll>(w + 1, 0));
    memo.resize(h + 1, vector<ll>(w + 1, -1));
