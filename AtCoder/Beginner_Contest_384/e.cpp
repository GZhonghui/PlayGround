...
int main()
{
    ...
    ull res = 0; // a[p][q];
    pq.push((node){a[p][q],p,q});
    vis.insert(mk(p,q));

    while(!pq.empty()) {
        node now = pq.top(); pq.pop();

        bigint::bigint _res = bigint::bigint(res);
        bigint::bigint _v = bigint::bigint(now.v);
        bigint::bigint _x = bigint::bigint(x);
        if(res == 0 || _res > _v * _x) {
            res += now.v;
            rep(i,0,3) {
                ull ii = now.i + dx[i];
                ull jj = now.j + dy[i];
                if(1 <= ii && ii <= h && 1 <= jj && jj <= w && !vis.count(mk(ii,jj))) {
                    vis.insert(mk(ii,jj));
                    pq.push((node){a[ii][jj],ii,jj});
                }
            }
        }
    }
