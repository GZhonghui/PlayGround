...
ll d[maxn][maxn], t, d2a[maxn];
bool is_airport[maxn];

void main_algo() {
    cin >> n >> m;
    memset(is_airport, false, sizeof(is_airport));
    ...
    auto add = [&](ll a, ll b, ll new_d) {
        ...
        rep(i, 1, n) {
            rep(j, i + 1, n) {
                ll _t1 = d[i][a] + new_d + d[b][j];
                ll _t2 = d[i][b] + new_d + d[a][j];
                ll _t3 = min(_t1, _t2);
                if(_t3 < d[i][j]) {
                    if(d[i][j] < inf) {
                        ans -= d[i][j] * 2;
