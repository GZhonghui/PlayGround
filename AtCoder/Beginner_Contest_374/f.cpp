...
ll n, k, x, t[maxn];
vector<ll> state_times, pre_state;
vector< vector<ll> > memo;

ll dp(ll state_id, ll order_id) {
    if(memo[state_id][order_id] != -1) return memo[state_id][order_id];
    ll &res = memo[state_id][order_id];

    if(order_id == 0) {
        return res = 0;
    }

    if(state_id == 0 || t[order_id] > state_times[state_id]) {
        return res = inf;
    }

    res = inf;
    if(state_id > 1) {
        res = min(res, dp(state_id - 1, order_id));
    }

    // if(pre_state[state_id] != 0) {
        ll _sum = 0;
        rep(i, 1, min(k, order_id)) {
            _sum += state_times[state_id] - t[order_id - i + 1];
            res = min(res, dp(pre_state[state_id], order_id - i) + _sum);
        }
    // }
...
int main()
{
    ...
    set<ll> possible_times;
    rep(i, 1, n) {
        rep(j, 0, n - 1) {
            possible_times.insert(t[i] + j * x);
        }
    }

    state_times.resize(possible_times.size() + 1, 0);
    for(ll t : possible_times) {
        state_times[++state_times[0]] = t;
        // cout << t << " ";
    }
    // cout << endl;

    pre_state.resize(state_times[0] + 1, 0);
    rep(i, 1, state_times[0]) {
        rep(j, 1, i - 1) {
            if(state_times[i] - state_times[j] >= x) {
                pre_state[i] = j;
            }
        }
    }

    memo.resize(state_times[0] + 1, vector<ll>(n + 1, -1));

    ll ans = dp(state_times[0], n);
    cout << ans << endl;
