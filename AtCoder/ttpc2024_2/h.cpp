...
int main()
{
    ...
    if(res[1] && res[2] && res[3] && res[4]) {
        ...
        if(lens.size() == 4) cout << "NeverEnds" << endl;
        else {
            ...
            for(ll _len : lens) {
                ...
                rep(i,1,4) if(loop_len[i] == _len) _selected.pb(i);
                if(_selected.size() <= 1) continue;

                ll _leader = _selected[0];
                ll res = 0;
                for(ll x : _s[_leader]) {
                    bool all_in = true;
                    for(ll _k : _selected) {
                        if(!_s[_k].count(x)) all_in = false;
                    }
                    if(all_in) {
                        res = max(res, x);
                    }
                }
                if(res < n) failed = true;
            }
            if(failed) {
                ll res = 0;
                for(ll x : _s[1]) {
                    if(_s[2].count(x) && _s[3].count(x) && _s[4].count(x)) res = max(res, x);
                }
                cout << res << endl;
