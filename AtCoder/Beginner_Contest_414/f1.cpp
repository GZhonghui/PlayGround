...
inline void solve() {
    ...
    dp[1][0] = 0;
    deque<node> Q;
    Q.push_back((node){1, -1, 0, 0});

    while(!Q.empty()) {
        node now = Q.front();
        Q.pop_front();
        
        if(now.steps != dp[now.id][now.substeps]) continue;

        // if(now.steps < dp[now.id][now.substeps]) {}

        ll substeps = (now.substeps + 1) % k;
        ll steps = dp[now.id][now.substeps] + (substeps == 0 ? 1 : 0);
        for(ll next : g[now.id]) {
            if(next == now.from) continue;

            if(steps <= dp[next][substeps]) {
                dp[next][substeps] = steps;
                if(substeps == 0) Q.push_back((node){next, -1, steps, substeps});
                else Q.push_front((node){next, now.id, steps, substeps});
            }
        }
    }
