...
const ll station_cost = 5000;
const ll road_cost = 100;
...
struct solution {
    ...
    void init(ll n, ll m, ll k, ll t, const vector<person> &people) {
        this->n = n;
        this->m = m;
        this->k = k;
        this->t = t;
    ...
    void print_ans() {
        for(auto &p : ans) {
            ll a = get<0>(p), b = get<1>(p), c = get<2>(p);
            if(a == -1) {
                cout << -1 << endl;
            } else {
                cout << a << " " << b << " " << c << endl;
...
struct solution_wait : public solution {
    virtual ll solve() {
        while(t--) {
            ans.pb({-1, 0, 0});
        }
...
struct solution_default : public solution {
    virtual ll solve() {
        return 0;
    }
};

struct solution_link_loop : public solution {
    ll loop_border;
...
int main()
{
    ...
    rep(i,1,m) {
        ll a, b, c, d;
        cin >> a >> b >> c >> d;
        people.pb((person){mk(a, b), mk(c, d), i});
    }
    ...
    // solutions.pb(make_shared<solution_wait>());
    // solutions.pb(make_shared<solution_default>());

    // ll best_ans = 0;
    // shared_ptr<solution> best_solution;
    // for(auto &s : solutions) {
    //     s->init(n, m, k, t, people);
    //     ll ans = s->solve();
    //     if(s->is_valid() && ans > best_ans) {
    //         best_ans = ans;
    //         best_solution = s;
    //     }
    // }
