...
ll dp(ll x, ll as_root) {
    if(memo[x][as_root] != -1) {
        return memo[x][as_root];
    }

    ll &res = memo[x][as_root];
    res = 0;

    if(g[x].size() == 1 && g[x][0] == fa[x]) {
        if(as_root) {
            return res = 0;
        } else {
            return res = 1;
    ...
    if(as_root) {
        res = 0;
        if(ans.size() >= 4) {
            res = max(res, ans[0] + ans[1] + ans[2] + ans[3] + 1);
        }
        if(max_v) {
            res = max(res, max_v + 1);
        }
        ...
        res = 1;
        if(ans.size() >= 3) {
            res = max(res, ans[0] + ans[1] + ans[2] + 1);
        }
    }
