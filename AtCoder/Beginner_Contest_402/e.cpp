...
const ll maxx = 5000 + 8;

ll x, s[maxn], c[maxn], p[maxn];

f dp[maxx], cache[maxx];

f memo[((ll)1) << 10][maxx];
bool v[((ll)1) << 10][maxx];

f dp_search(ll mask, ll money) {
    if(v[mask][money]) {
        return memo[mask][money];
    }

    f &res = memo[mask][money] = 0;
    v[mask][money] = true;
    ...
    rep(i, 1, n) {
        ...
        f local = dp_search(mask, money - c[i]) * (1.0 - p[i] * 0.01);
        local += (dp_search(mask | (((ll)1) << (i - 1)), money - c[i]) + s[i]) * (p[i] * 0.01) ;
...
void main_algo() {
    ...
    // dp[0] = 0;
    // rep(i, 1, maxx - 1) dp[i] = -inf;

    // rep(i, 1, n) {
    //     memcpy(cache, dp, sizeof(dp));
    //     rep(j, 0, x) {
    //         ll cost = 0;
    //         f t = 1.0;
    //         while(cost <= j) {
    //             cache[j] = max(cache[j], dp[j - cost] + s[i] * (1.0 - t));
    //             t *= (1.0 - p[i] * 0.01);
    //             cost += c[i];
    //         }
    //     }
    //     memcpy(dp, cache, sizeof(cache));
    // }
    ...
    cout << fixed << setprecision(10) << dp_search(0, x) << endl;
}