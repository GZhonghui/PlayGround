...
ll A[maxn][maxn], pow10[128];
vector<ll> pre[maxn], suff[maxn];

void dfs1(ll i, ll j, ll now, ll step) {
    if(step == n - 1) {
        pre[i].push_back(now);
        return;
    }
    if(i < n)
        dfs1(i + 1, j, (now * 10 + A[i + 1][j]) % m, step + 1);
    if(j < n)
        dfs1(i, j + 1, (now * 10 + A[i][j + 1]) % m, step + 1);
}

void dfs2(ll i, ll j, ll now, ll step, ll len) {
    if(step == n - 1) {
        suff[i].push_back(now);
        return;
    }

    ll t = (A[i][j] * pow10[len] + now) % m;

    if(i > 1)
        dfs2(i - 1, j, t, step + 1, len + 1);
    if(j > 1)
        dfs2(i, j - 1, t, step + 1, len + 1);
}
...
void main_algo() {
    ...
    pow10[0] = 1 % m;
    rep(t, 1, 2 * n) pow10[t] = (pow10[t - 1] * 10) % m;

    dfs1(1, 1, A[1][1] % m, 0);
    dfs2(n, n, 0, 0, 0);

    ll C = pow10[n - 1], ans = 0;

    rep(row, 1, n) {
        if (pre[row].empty() || suff[row].empty()) continue;
        ...
        for (ll a : pre[row]) {
            ...
            ll b = (it == S.begin()) ? S.back() : *prev(it);
