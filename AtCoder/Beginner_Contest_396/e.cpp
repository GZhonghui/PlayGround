...
ll n, m, ans[maxn], value[maxn];
bool vis[maxn];
vector<ll> group;

bool dfs(ll x, ll now) {
    vis[x] = 1;
    value[x] = now;
    ...
    for(ll eid : g[x]) {
        edge &e = edges[eid];
        if(vis[e.to]) {
            if(value[e.to] != (now ^ e.dis)) return false;
        } else {
            if(!dfs(e.to, now ^ e.dis)) return false;
        }
    }
    ...
    rep(i,1,n) {
        if(!vis[i]) {
            ...
            rep(i, 1, cnt) {
                ll cnt0 = 0, cnt1 = 0;
                for(ll x : group) {
                    if(value[x] & (1LL << (i - 1))) cnt1++;
                    else cnt0++;
                }

                if(cnt1 > cnt0) {
                    for(ll x : group) {
                        // value[x] = value[x] ^ (value[x] | (1LL << (i - 1)));
                        if(value[x] & (1LL << (i - 1))) {
                            value[x] = value[x] & (~(1LL << (i - 1)));
                        } else {
                            value[x] = value[x] | (1LL << (i - 1));
                        }
                    }
