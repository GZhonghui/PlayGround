...
ll n,k,a[maxn],cnt[maxn],beishu_cnt[maxn],ans[maxn];
...
int main()
{
    ...
    memset(beishu_cnt, 0, sizeof(beishu_cnt));
    rep(i,1,1e6) {
        ll x = i;
        while(x <= 1e6) {
            beishu_cnt[i] += cnt[x];
    ...
    //     for(ll i=divisors[number].size() - 1; i >= 0; i--) {
    //         ll d = divisors[number][i];
    //         if(beishu_cnt[d] >= k) {
    //             cout << d << endl;
    //             ans[number] = d;
    //             break;
