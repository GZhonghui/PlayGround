...
ll n, cnt[maxn],pos[maxn],pos_sum[maxn];
char s[maxn];
...
int main()
{
    ...
    vector<S_both> initial(n + 1, {0, 0, 1}); // avaliable range: [0, n + 1)
    ...
    ll res = inf;
    while(r <= n) {
        ll max_right = seg.max_right(1, [&](S_both s) {
            return s.len - l + 1 <= s.sum;
        }) - 1;
        ll min_left = max_right + 1;
        ll left_cnt = max_right - l + 1, right_cnt = r - min_left + 1;

        // cout << l << " " << r << " " << max_right << endl;

        ll _res = 0;
        _res += sum_range(max_right - left_cnt + 1, max_right) - pos_sum[max_right];
        _res += pos_sum[n] - pos_sum[min_left - 1] - sum_range(min_left, min_left + right_cnt - 1);
        // cout << "l = " << sum_range(max_right - left_cnt + 1, max_right) << " " << pos_sum[max_right] << endl;
        // cout << "r = " << pos_sum[n] << " " << pos_sum[min_left - 1] << " " << sum_range(min_left, min_left + right_cnt - 1) << endl;
        // cout << _res << endl;
