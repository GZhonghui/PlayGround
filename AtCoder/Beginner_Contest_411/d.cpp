...
ll q;
vector<string> ss;
ll server_s = -1, server_e = -1, nodes[maxn][2], nxt[maxn], stk[maxn];
...
void main_algo() {
    ...
    ll top = 0;
    for(ll cur = server_s; cur != -1; cur = nxt[cur]) stk[top++] = cur;
    rre(i, 0, top - 1) cout << ss[stk[i]];
    cout << endl;
}
...
void main_algo_t1() {
    ...
    while(q--) {
        ...
        if(tp == 1) {
            ...
            ss.pb(s);
            ll id = ss.size() - 1;
            if(nodes[p][0] == -1 && nodes[p][1] == -1) {
                nodes[p][0] = nodes[p][1] = id;
            } else {
                nxt[nodes[p][1]] = id;
                nodes[p][1] = id;
            }
    ...
    // rep(i, 0, ss.size() - 1) {
    //     cout << i << ": " << ss[i] << " -> " << nxt[i] << endl;
    // }

    cout << server_s << " - " << server_e << endl;

    if(server_s != -1 && server_e != -1) {
        while(server_s != server_e) {
            cout << ss[server_s];
            server_s = nxt[server_s];
        }
        cout << ss[server_s] << endl;
    }
}