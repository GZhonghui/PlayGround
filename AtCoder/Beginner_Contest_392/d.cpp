...
struct node {
    ...
    inline f calc(node &r) {
        f ans = 0;
        for(auto x : m) {
            if(r.m.count(x.first)) {
                ans += 1.0 * x.second * r.m[x.first] / (k * r.k);
            }
        }
        return ans;
    }
...
int main()
{
    ...
    rep(i,1,n) {
        rep(j,i+1,n) {
            ans = max(ans, nodes[i].calc(nodes[j]));
        }
    }
